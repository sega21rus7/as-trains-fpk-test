...
- После выполнения каждой задачи запускать проект и проверять, что он работает.
- Перед завершением задачи выполнять все применимые автоматические проверки: `npm test`, `npm run lint` и `npm run build`.
